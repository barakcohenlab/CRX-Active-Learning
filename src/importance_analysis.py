...
def main(model_dir, output_dir, seed, dev, valid_positions):
    ...
    #pwms = predicted_occupancy.read_pwm_files("avi.meme")
    ...
    logger.info(f"Ordered list of positions features will be inserted into: {valid_positions}")
    for i, combo in enumerate(get_motif_combos(consensus_seqs)):
        combo_str = ",".join(combo)
        logger.info(f"Testing sequences with the following order of insertions: {combo_str}")
        inserted_seqs = add_features(shuffled_seqs, consensus_seqs[combo].values.tolist(), valid_positions)
        log_sequences(logger, inserted_seqs)
        # Generate the encoding string for the combinations added. Each motif is encoded as the inserted position and
        # then the TF name. Motifs are comma-delimited and sorted in numerical order.
        encoding_info = dict()      # {insertion position: motif name}
        for tf, loc in zip(combo, valid_positions):
            encoding_info[loc] = tf
        sorted_positions = sorted(list(encoding_info.keys()))
        encoding = ",".join([f"{pos}{encoding_info[pos]}" for pos in sorted_positions])

        shuffled_df[encoding] = cnn_predict(inserted_seqs, model, use_cuda=use_cuda)

    logger.info(f"Tested {i+1} motif combinations, scores to file.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid_positions", nargs="+", type=int,
                        help="Ordered positions to insert into.",
                        default=[78, 98, 58, 118, 38, 138, 18],
                        )
