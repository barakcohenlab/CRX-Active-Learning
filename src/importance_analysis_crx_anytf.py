...
from mpra_tools import fasta_utils, log, loaders, modeling
...
def main(model_dir, tf, tf_motif, output_dir, output_prefix, seed, dev):
    ...
    for crx_position in range(len(shuffled_seqs[0]) - len(crx_motif) + 1): #Assumes all shuffled are of same lengths
        ...
        for tf_position in range(len(shuffled_seqs[0]) - len(tf_motif) + 1): #Assumes all shuffled are of same lengths
            if tf_position <= crx_position  - len(tf_motif) or tf_position >= crx_position + len(crx_motif):
                i += 1
                crx_tf_seqs = add_features(crx_seqs, tf_motif, tf_position)
                logger.info(f"Testing sequences with the following crx, {tf} position : {crx_position}, {crx_motif}, {tf_position}, {tf_motif}")
                log_sequences(logger, crx_tf_seqs)
                encoding = f"{crx_position}crx{tf_position}{tf}"
                shuffled_df[encoding] = cnn_predict(crx_tf_seqs, model, use_cuda=use_cuda)

    logger.info(f"Tested {i} motif combinations, scores to file.")
    shuffled_df.to_csv(os.path.join(output_dir, f"{output_prefix}_synthetic_insertion_preds.txt"), sep="\t")
...
if __name__ == "__main__":
    ...
    parser.add_argument("tf", help="Name of TF used in perturbation")
    parser.add_argument("tf_motif", help="Motif of TF used in perturbation")
